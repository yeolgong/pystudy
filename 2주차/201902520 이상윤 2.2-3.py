#답안지와 배점을 주고 최고성적 및 id를 출력하는 프로그램
...
maxscoreid.sort()   #id를 작은순서대로 정렬
for i in range(len(maxscoreid)):
    print(maxscoreid[i], end=' ')   #id(리스트)를 공백을 두고 한줄로 출력
...
for i in range(people):
    ...
    for j in range(answerslen):
        if list[i][j+1]==answers[j]:
            okcount[j]=okcount[j]+1
    
    
for i in range(answerslen):
    okrate[i]=okcount[i]/people*100
...
for i in range(answerslen):     
    if okcount[i]==lowcount:       
        print("%d" %(i+1), end=' ')
        '''
'''컴퓨터는 실수를 비트로 정확하게 표현하지 못해서 정답률로 비교하는 것보다는
제일 적게 맞춘 문제를 비교함'''
